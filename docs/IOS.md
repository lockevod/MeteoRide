...
mobile/
  ...
  native/ios/               Swift, plist fragments and the two privacy manifests
...
Two of the app's own must appear — `App.app/PrivacyInfo.xcprivacy` and
`App.app/PlugIns/ShareExtension.appex/PrivacyInfo.xcprivacy` — alongside the empty
ones inside `Frameworks/`. Then run *Validate App* in the Organizer.
...
4. Add `mobile/native/ios/ShareExtension/PrivacyInfo.xcprivacy` to the extension
   target, referenced in place. An app extension is a bundle of its own and Apple
   reads its manifest separately; the App target's does not cover it. This one is
   shorter — the extension compiles only `MeteoRideShareStore.swift` and has no
   Capacitor plugins — and the file says why.
5. *Signing & Capabilities* → same Team, then App Groups → `group.cc.meteoride.app`.
...
6. Set the extension's deployment target to the same value as the App target —
