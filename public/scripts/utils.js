(function() {
  ...
  const i18n = {
    es: {
      ...
      provider_rejected: "El proveedor ha rechazado la petición. Revisa tu clave API en ajustes.",
  ...
  recent_routes_count: "{n} rutas recientes",
      ...
      provider_key_missing: "{prov} requiere una clave API.",
      provider_key_invalid: "Clave API no válida para {prov}.",
      ...
      fallback_short: "Se usa Open‑Meteo.",
      // NEW: API key tester strings
      check_key: "Comprobar",
      key_test_missing: "Introduce primero la clave API.",
      key_testing: "Probando...",
      key_valid: "Clave API válida.",
      key_invalid: "Clave API no válida o sin permiso.",
     ...
     show_weather_alerts_needs_key: "Requiere una clave API de OpenWeather.",
  ...
  title: "MeteoRide — Forecast for your ride",
      ...
      units_label: "Units and language",
  ...
  function logDebug(msg, isError = false) {
    ...
    // The panel starts closed through the hidden attribute, which leaves style.display empty,
    // so only the computed style says whether anyone can see it.
    const panelOpen = !!dbgSection && getComputedStyle(dbgSection).display !== 'none';
    // Debug is visible if the panel is open OR the settings checkbox asks for debug output
    const debugVisible = !!(sdb && sdb.checked) || panelOpen;
