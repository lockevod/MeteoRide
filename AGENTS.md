...
| | iOS | Android |
|---|---|---|
| Receives the file | `SceneDelegate`, via `CFBundleDocumentTypes` | `MainActivity` intent filters |
| Inbox | App Group container | app private files dir |
| Wakes the web layer | `sharedRouteAvailable` plugin event | `sharedRouteAvailable` plugin event |

**iOS ships no share extension, and adding one breaks sharing.** Both mechanisms put an
identical-looking MeteoRide in the share sheet, the extension's icon is the one that gets
tapped, and a share extension cannot open its containing app. Removing it is what made
sharing a route from another app open MeteoRide, confirmed on a device on 18/09/2026.
docs/IOS.md step 5 has the full account; `mobile/tests/document-open.test.mjs` pins it.
...
needs App Group capabilities and target membership that only exist inside the
`.pbxproj`, and those cannot be produced reliably outside Xcode. So the iOS Swift
sources live in `mobile/native/ios/` and `docs/IOS.md` lists the one-time steps
to wire them up. If you find a dependable way to script that, committing
...
- **The store refuses anything that is not a file URL, and still should.** The guard
  dates from the share extension, whose activation rule accepted any `public.data`
  attachment, so a link shared from Strava, Komoot or a browser arrived as a URL item —
  and `Data(contentsOf:)` accepts an https URL and performs a blocking, untimed download.
  The document path cannot deliver a web URL, so the exposure is gone with the extension,
  but `ingest` is reachable from anything that gets a URL into `SceneDelegate` and the
  guard costs one line. Android
...
- **iOS reads a shared file in chunks and stops at the cap.** `Data(contentsOf:)`
  loaded the whole file before the 25 MB check. `SceneDelegate` also reads one URL at a
  time, on a serial queue, so two routes opened together cost one ceiling and not two.
- **Two shares landing in the same millisecond need a tiebreaker, and an atomic write
  ...
  **iOS adds a third field**, `<millis>-<sequence>-<8 hex of a UUID>__<name>`
  ...
  counter is a process-local `NSLock` and `var`, and on iOS *two* processes used to write
  this folder — the app and the share extension, each starting its own counter at zero over
  the same App Group container. Same millisecond plus same count meant the same name, and
  `.atomic` overwrites rather than refusing, so one of the two routes vanished silently.
  The extension is gone and one process writes there now, so the field is no longer load
  bearing; it is kept because removing it would rename files an installed app may already
  hold, for nothing.
...
- **The privacy manifests' reason codes are exactly the ones the plugin READMEs give,
  ...
  `mobile/native/ios/PrivacyInfo.xcprivacy` is tracked because `mobile/ios/` is not, and
  Apple rejects an upload that touches a required-reason API without declaring it. The
  codes are `C617.1` for file timestamps and `CA92.1` for user defaults.
  ...
  **Read Apple's own text before changing a code, and read it from the documentation
  ...
  Verified by unsigned archive on 17/09/2026, when there was still a share extension:
  each manifest landed at its bundle root. Only `App.app/PrivacyInfo.xcprivacy` remains.
...
- **No app extension can open its containing app**, which is why there is no share
  extension. `NSExtensionContext.open` is given to Today widgets only, and iOS 18 refuses
  the responder-chain `openURL:` workaround out loud. Routes arrive through
  `CFBundleDocumentTypes` instead, which launches the app.
...
- **In**: `CFBundleDocumentTypes` on iOS, the intent filters on Android. Whatever the
  other app exports as a `.gpx`/`.kml` file, we accept. A shared *link* is refused on
  purpose — those services gate their downloads behind a session, so a URL would fetch a
  login page. Whatever the
...
- **No ride alert has ever fired on a physical device.** iOS background tasks never
  execute in the simulator, so the delivery path has never run for real: the rules and
  the runner are covered by tests, the delivery is not. (Sharing a route in HAS been
  tested on a device, 18/09/2026; that is the only thing a real iPhone has settled.)
